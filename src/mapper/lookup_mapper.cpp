...
std::vector<double> LookupMapper::weighted_textgcn_embedding(std::vector<long>& indexes, std::vector<double> weights, int num_to_agg) {
    std::vector<double> avg_embedding(this->dimension, 0.0);
    double size = indexes.size()-1;
    if (size)
    {
        int counter = 0;
        //for (auto it=++indexes.begin(); it!=indexes.end(); it++)
        for (int i=1; i<=num_to_agg; i++)
            for (int d=0; d<this->dimension; d++)
                //avg_embedding[d] += (this->embedding[*(it)][d] * weights[counter]);
                avg_embedding[d] += this->embedding[indexes[i]][d] * weights[i-1];
            counter += 1;
        for (int d=0; d<this->dimension; d++)
        {
            avg_embedding[d] = (this->embedding[indexes[0]][d] + avg_embedding[d]/size)/2.0;
